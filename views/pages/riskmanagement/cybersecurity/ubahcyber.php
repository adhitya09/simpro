...
// Ambil ID dari URL
$id_cyber = $_GET['id'] ?? null;
if (!$id_cyber) {
	die('<div class="alert alert-danger">ID not specified.</div>');
}

// Ambil data dari database berdasarkan ID
$query = mysqli_query($koneksi, "SELECT * FROM cyber WHERE id_cyber = '$id_cyber'");
...
if (isset($_POST['simpan'])) {
	...
	if (empty($date) || empty($event) || empty($risk) || empty($mitigation) || empty($prg) || empty($status)) {
		...
		$updateQuery = mysqli_query($koneksi, "UPDATE cyber SET 
            date = '$date', 
            event = '$event', 
            risk = '$risk', 
		...
		<div class="card-body">
			<form method="post" enctype="multipart/form-data" class="form-horizontal">
				<div class="form-group row">
					...
					<div class="col-sm-4">
						<input type="date" class="form-control" id="date" name="date" value="<?php echo htmlspecialchars($data['date']); ?>" required>
					</div>
				</div>
				<div class="form-group">
					<label class="form-label" for="summernote1">Unexpected Event</label>
					<textarea class="form-control" id="summernote1" rows="5" name="event" required><?php echo htmlspecialchars($data['risk']); ?></textarea>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<textarea class="form-control" id="summernote3" rows="5" name="mitigation" required><?php echo htmlspecialchars($data['prg']); ?></textarea>
				</div>
				<div class="form-group">
					<label class="form-label" for="summernote4">Progress</label>
					<textarea class="form-control" id="summernote4" rows="5" name="prg" required><?php echo htmlspecialchars($data['prg']); ?></textarea>
				</div>
				<div class="form-group row">
					<label class="control-label col-sm-2 align-self-center mb-0" for="status">Status:</label>
					<div class="col-sm-4">
						<select name="status" class="form-select shadow-none">
							<option value="1" class="text-primary" <?php if ($data['status'] == 1) echo 'selected'; ?>>Open</option>
							<option value="2" class="text-info" <?php if ($data['status'] == 2) echo 'selected'; ?>>In Progress</option>
							<option value="3" class="text-secondary" <?php if ($data['status'] == 3) echo 'selected'; ?>>Hold</option>
							<option value="4" class="text-success" <?php if ($data['status'] == 4) echo 'selected'; ?>>Done</option>
							<option value="5" class="text-danger" <?php if ($data['status'] == 5) echo 'selected'; ?>>Cancel</option>
						</select>
					</div>
				</div>
